...
// ── 2. FINANCIAL RISK ─────────────────────────────────────────────────────
  if (daily === 4 || monthly === 4) {
    ...
    findings.push(DT?.financialCautionLayer
      ...
      : { type: 'financial', level: isToday ? 'high' : 'medium', _dyn: true, label: 'Financial Caution', detail: `Rahu energy active ${isToday ? 'today' : 'this month'}. Avoid financial commitments — verify every opportunity twice. What looks like a sure thing may not be.` });
  }
  if (natalNums.includes(4) && daily === 5) findings.push({ type: 'financial', level: 'medium', label: 'Spending Impulse', detail: 'Rahu in your natal chart on a Mercury day. Impulsive spending tendency peaks — the urge to spend is strong and potentially impulsive.' });
  if (basic === 4 && maha === 5) findings.push({ type: 'financial', level: 'medium', label: 'Financial Period Caution', detail: 'Rahu natal in a Mercury period. Business opportunities may look more certain than they are.' });
  if (daily === 4 && antar === 5) findings.push({ type: 'financial', level: 'medium', label: 'Financial Caution', detail: 'Rahu day in a Mercury chapter. Sharp financial instinct today but verification is essential.' });
  ...
  if (daily === 8 && maha === 8) findings.push({ type: 'health', level: 'medium', label: 'Physical Fatigue Risk', detail: 'Double Saturn. The body is under maximum pressure today. Rest is as important as effort.' });
  ...
  if (maha === 4 && daily === 9) findings.push({ type: 'legal', level: 'high', label: 'Legal Caution', detail: 'Mars day in a Rahu period. This specific combination is linked to legal risks from aggressive impulsive action.' });
  if (daily === 9 && antar === 4) findings.push({ type: 'legal', level: 'medium', label: 'Anger Caution', detail: 'Mars day in a Rahu chapter. Frustration levels are elevated — confrontations can escalate beyond intention.' });
  ...
  if (daily === 6 && maha === 9) findings.push({ type: 'relationship', level: 'medium', label: 'Sharp Tongue Risk', detail: 'Venus day in a Mars period. Words said in frustration today carry unusual weight and last longer than usual.' });
  ...
  if (daily === 1 && maha === 7) findings.push({ type: 'opportunity', level: 'high', label: 'Lucky Authority', detail: 'Sun day in a Ketu period. Bold moves are backed by quiet fortune today. Make the ask.' });
  if (daily === 7 && maha === 1) findings.push({ type: 'opportunity', level: 'high', label: 'Fortune Backs Initiative', detail: 'Ketu day in a Sun period. What you initiate today has unusual fortune behind it.' });
  ...
  if (daily === 7 && natalNums.includes(7)) findings.push({ type: 'spiritual', level: 'high', label: 'Double Ketu', detail: 'Ketu in both natal and day. Maximum intuition and inner depth available. Trust instinct completely.' });
  if (daily === 3 && basic === 3) findings.push({ type: 'spiritual', level: 'high', label: 'Double Jupiter', detail: 'Jupiter in both natal and day. Clearest judgment and sound reasoning of the cycle available today.' });
  ...
  // ── 8. KARMA DAYS ─────────────────────────────────────────────────────────
  if (daily === 8 && maha === 8) findings.push({ type: 'karma', level: 'high', label: 'Double Saturn', detail: 'Maximum effort day. What is built with integrity today compounds. Shortcuts cost triple. The effort matters.' });
  if (daily === 8 && basic === 8) findings.push({ type: 'karma', level: 'high', label: 'Saturn Day for Saturn Natal', detail: 'Your most consequential day type. The work done today has the longest-lasting consequences — good or bad.' });
  if (natalNums.filter(n => n === 8).length >= 2 && daily === 8) findings.push({ type: 'karma', level: 'high', label: 'Extreme Saturn Day', detail: 'Multiple 8s in chart on a Saturn day. This is a maximum-effort day. Act with complete integrity.' });
